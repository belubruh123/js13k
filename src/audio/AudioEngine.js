...
export class AudioEngine{
  ...
  /** Low buzzing hum for cursed menu */ buzz(on=true){ this.ensure(); if(!on){ this.stopBed('buzz'); return; } if(this.runningBeds.has('buzz')) return; const ac=this.ac; const g=ac.createGain(); g.gain.value=0.08; const osc=ac.createOscillator(); osc.type='sawtooth'; osc.frequency.value=60; const sh=ac.createWaveShaper(); sh.curve=new Float32Array([ -1,-0.5,0,0.5,1 ]); osc.connect(sh).connect(g).connect(this.master); osc.start(); this.runningBeds.set('buzz', ()=>{ g.disconnect(); osc.stop(); }); }
  ...
  /** Quick static burst */ staticBurst(dur=0.2){ this.ensure(); const ac=this.ac; const b=ac.createBuffer(1, ac.sampleRate*dur, ac.sampleRate); const d=b.getChannelData(0); for(let i=0;i<d.length;i++) d[i]=(Math.random()*2-1)*0.5; const src=ac.createBufferSource(); src.buffer=b; const g=ac.createGain(); g.gain.value=0.2; src.connect(g).connect(this.master); src.start(); src.stop(ac.currentTime+dur); }
}
